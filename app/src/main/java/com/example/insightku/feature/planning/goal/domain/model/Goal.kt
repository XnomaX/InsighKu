...
data class Goal(
    ...
    val isPaused: Boolean get() = status == GoalStatus.PAUSED
    val isArchived: Boolean get() = status == GoalStatus.ARCHIVED
    val daysRemaining: Int? get() = deadline?.let {
        val today = LocalDate.now()
        if (it.isBefore(today)) 0 else java.time.temporal.ChronoUnit.DAYS.between(today, it).toInt()
    }
    val isOverdue: Boolean get() = deadline != null && deadline.isBefore(LocalDate.now()) && !isCompleted
...
data class GoalSummary(
    val totalGoals: Int, val activeGoals: Int, val completedGoals: Int,
    val totalSaved: Double, val totalTarget: Double, val overallProgress: Double
)
