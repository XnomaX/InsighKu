...
class UserPreferencesDataStore @Inject constructor(
    ...
    val appLanguage: Flow<String> = context.dataStore.data.map { preferences ->
        preferences[APP_LANGUAGE_KEY] ?: "id"
    }
    ...
    private val BANK_NOTIFICATION_KEY = booleanPreferencesKey("bank_notification_enabled")

    val bankNotificationEnabled: Flow<Boolean> = context.dataStore.data.map { preferences ->
        preferences[BANK_NOTIFICATION_KEY] ?: false
    }

    suspend fun setBankNotificationEnabled(enabled: Boolean) {
        context.dataStore.edit { preferences ->
            preferences[BANK_NOTIFICATION_KEY] = enabled
        }
    }

    // ── Consent whitelist: package aplikasi keuangan yang user IZINKAN diproses ──
    // CONSENT terpisah dari CAPABILITY (parser yg kita dukung). Default KOSONG:
    // sistem tidak memproses notifikasi apa pun sampai user memilih secara eksplisit.
    private val ALLOWED_BANK_PACKAGES_KEY = stringSetPreferencesKey("allowed_bank_packages")

    /** Set package yang diizinkan user. Default kosong. */
    val allowedBankPackages: Flow<Set<String>> = context.dataStore.data.map { preferences ->
        preferences[ALLOWED_BANK_PACKAGES_KEY] ?: emptySet()
    }

    /** Izinkan satu package (idempotent). */
    suspend fun allowBankPackage(packageName: String) {
        if (packageName.isBlank()) return
        context.dataStore.edit { preferences ->
            preferences[ALLOWED_BANK_PACKAGES_KEY] =
                (preferences[ALLOWED_BANK_PACKAGES_KEY] ?: emptySet()) + packageName
        }
    }

    /** Cabut izin satu package (idempotent). */
    suspend fun disallowBankPackage(packageName: String) {
        context.dataStore.edit { preferences ->
            preferences[ALLOWED_BANK_PACKAGES_KEY] =
                (preferences[ALLOWED_BANK_PACKAGES_KEY] ?: emptySet()) - packageName
        }
    ...
    /** Set seluruh whitelist sekaligus (mis. saat aktivasi onboarding). */
    suspend fun setAllowedBankPackages(packages: Set<String>) {
        context.dataStore.edit { preferences ->
            preferences[ALLOWED_BANK_PACKAGES_KEY] = packages
        }
    }
}
