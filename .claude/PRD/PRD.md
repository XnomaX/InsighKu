...
|--------|--------------------------|-----------------------|----------|-------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------|
| BUG-03 | Data safety              | P0 — Highest Priority | Resolved | Destructive Room migration could erase participant data.                | `DatabaseModule.kt` previously used `fallbackToDestructiveMigration(true)`; removed. App was never deployed → Room DB reset to **version 1**, all `Migration_*` code deleted. | Remove fallback; reset schema to v1 with no migration chain. **Done** — schema baseline at `app/schemas/.../1.json`. |
